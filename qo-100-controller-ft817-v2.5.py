# PA3ANG, June - 2020
version	= "2.5"
# version 2.5   version for smaller 7" screen  with less knobs. 
#
...
CMD_UNLOCK          = [0x00, 0x00, 0x00, 0x00, 0x80]
...
# title bar
window.wm_title("FT-817 / QO-100  | Mode: "+satmode+" | Port: "+SERIAL_PORT+" | Version: "+version)
...
def store_mem ():
    # this sub is called once
    global QO_frequency, Mem_frequency
    Mem_frequency = QO_frequency
    button_spare.configure(highlightbackground="green")
    button_spare.configure(command = restore_mem, text=(Mem_frequency-1048900000)/100)

def restore_mem ():
    global QO_frequency, Mem_frequency
    Mem_temp_frequency = QO_frequency
    set_frequency(Mem_frequency)
    Mem_frequency=Mem_temp_frequency
    button_spare.configure(text=(Mem_frequency-1048900000)/100)
...
def read_clar ():
    ...
    if (resp == b'\x00') :
        # clarifier pressed
        # and by command switched off again
        return True
    else:
        # nothing
        return False
...
def read_frequency ():
    ...
    if read_lock() is True:
        update_tx_frequency()
    
    if read_clar() is True:
        if setcal is False:
            set_bcn()
        else:
            calibrate()
    ...
    if read_ptt() is False:
        ...
        button_tune.configure(highlightbackground=window["bg"])
        button_tune.configure(text = "Tune")

    else:
        button_tune.configure(highlightbackground="red")
        button_tune.configure(text = "TXng")
...
def update_tx_frequency ():
    ...
    ser.close()
    button_update.configure(text=(QO_frequency-1048900000)/100)
...
label_frequency.grid(row=1, column=2, padx=(16,6))

# function keys

button_calibrate = Button(window, command = set_bcn, width=5, highlightthickness=2)
button_calibrate.grid(column=0, row=1, ipady=6, padx=(2, 2))
button_spare     = Button(window, text = "Mem", command = store_mem, width=5, highlightthickness=2)
button_spare.grid(column=1, row=1, ipady=6, padx=(2, 2))
button_tune      = Button(window, text = "Tune", command = start_tune, width=5, highlightthickness=2)
button_tune.grid(column=3, row=1, ipady=6, padx=(2, 2))
button_update    = Button(window, text = "Up.TX", command = update_tx_frequency, width=5, highlightthickness=2, highlightbackground="red")
button_update.grid(column=4, row=1, ipady=6, padx=(2, 2))

Button(window, text = "525/CW" , command  = set_525,    width=5).grid(column=5, row=1, ipady=6, padx=(2, 2))
#Button(window, text = "550/DIG", command = set_550,     width=6).grid(column=1, row=3, ipady=10, pady=(4, 4), padx=(6, 6))
Button(window, text = "680/USB", command = set_680,     width=5).grid(column=6, row=1, ipady=6, padx=(2, 2))
#Button(window, text = "850/USB", command = set_850,     width=6).grid(column=3, row=3, ipady=10, pady=(4, 4), padx=(6, 6))
